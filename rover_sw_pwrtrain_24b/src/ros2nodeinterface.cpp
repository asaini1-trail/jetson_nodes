...
// C/C++ source code generated on : Mon Dec 22 21:21:53 2025
//
#ifdef _MSC_VER
#pragma warning(push)
...
const std::string SLROSNodeName("rover_sw_pwrtrain_24b");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("rover_sw_pwrtrain_24b");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"rover_sw_pwrtrain_24b\" **\n");
        mExec = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
        mExec->add_node(SLROSNodePtr);
        //initialize the model which will initialize the publishers and subscribers
        mModel = std::make_shared<rover_sw_pwrtrain_24b>(
        );
