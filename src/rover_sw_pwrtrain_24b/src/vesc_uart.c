...
/* Helper to send duty to a specific CAN ID via the master VESC */
static int vesc_send_duty_can(int fd, uint8_t can_id, int32_t duty_val) {
    uint8_t can_p[7] = {
        COMM_FORWARD_CAN,
        can_id,
        COMM_SET_DUTY,
        (uint8_t)(duty_val >> 24), (uint8_t)(duty_val >> 16),
        (uint8_t)(duty_val >> 8),  (uint8_t)duty_val
    };
    int ret = vesc_send_payload_fd(fd, can_p, sizeof(can_p));
    tcdrain(fd);
    usleep(2000);  /* Give CAN bus time to transmit */
    return ret;
}
...
int vesc_send_duty_left(double duty) {
    ...
    /* Forward to slave VESCs via CAN first */
    for (size_t i = 0; i < sizeof(VESC_CAN_LEFT_SLAVES); i++) {
        if (vesc_send_duty_can(g_fd_left, VESC_CAN_LEFT_SLAVES[i], v) != 0) {
            ret = -1;
        }
    }

    /* Send to master VESC last */
    uint8_t p[5] = {
        COMM_SET_DUTY,
        (uint8_t)(v >> 24), (uint8_t)(v >> 16),
        (uint8_t)(v >> 8),  (uint8_t)v
    };
    if (vesc_send_payload_fd(g_fd_left, p, sizeof(p)) != 0) {
        ret = -1;
    }
...
int vesc_send_duty_right(double duty) {
    ...
    /* Forward to slave VESCs via CAN first */
    for (size_t i = 0; i < sizeof(VESC_CAN_RIGHT_SLAVES); i++) {
        if (vesc_send_duty_can(g_fd_right, VESC_CAN_RIGHT_SLAVES[i], v) != 0) {
            ret = -1;
        }
    }
    ...
    if (vesc_send_payload_fd(g_fd_right, p, sizeof(p)) != 0) {
        ret = -1;
    }
